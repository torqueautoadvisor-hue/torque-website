...
import { updateHisabRecord } from '../../../actions/finance';
...
export default function HisabEditForm({
  record,
  paymentMethods,
  statusList,
}: {
  ...
  const [date, setDate] = useState(formatDate(record.dl_hsb_date) || new Date().toISOString().split('T')[0]);
  const [incexp, setIncexp] = useState(record.dl_hsb_incexp ? String(record.dl_hsb_incexp) : '');
  const [pmId, setPmId] = useState(record.pm_id ? String(record.pm_id) : '');
  const [amount, setAmount] = useState(record.dl_hsb_amount ? String(record.dl_hsb_amount) : '');
  const [name, setName] = useState(record.dl_hsb_name || '');
  const [regno, setRegno] = useState(record.dl_hsb_regno || '');
  const [description, setDescription] = useState(record.dl_hsb_description || '');
  const [status, setStatus] = useState(record.dl_hsb_status ? String(record.dl_hsb_status) : '1');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateHisabRecord(record.dl_hsb_id, formData);
      if (res.success) {
        router.refresh();
        if (res.redirect) router.push(res.redirect);
      } else {
        setError(res.error || 'Failed to update daily hisab record.');
      }
    } catch (err: any) {
  ...
  return (
    <div>
      <div className="pageheader">
        <h2><i className="fa fa-edit"></i> Edit Daily Hisab</h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/insurance">Dashboard</a></li>
            <li><a style={{ color: '#1C1B17' }} href="/daily-hisab">Daily Hisab</a></li>
            <li className="active">Edit Daily Hisab</li>
          </ol>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="panel panel-default">
                <div className="panel-heading">
                  <h4 className="panel-title">Edit Daily Hisab Details ({record.dl_hsb_code_no})</h4>
                ...
                <div className="panel-body">
                  ...
                  <div className="form-group col-md-12 col-lg-12 col-sm-12 col-xs-12" style={{ marginBottom: '15px' }}>
                    ...
                    <div className="col-sm-9">
                      <input type="text" disabled className="form-control" value={record.dl_hsb_code_no || ''} />
                    </div>
                  </div>
                    ...
                    <div className="col-sm-9">
                      {record.dl_hsb_image && (
                        <div style={{ marginBottom: '10px' }}>
                          <a href={record.dl_hsb_image} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'underline', color: 'green', fontSize: '13px' }}>
                ...
                <div className="panel-footer">
                  <div className="row">
                    <div className="col-sm-12 col-lg-12 col-md-12 col-xs-12 ml_15">
                      ...
                      <button type="button" className="btn btn-default" onClick={() => router.push('/daily-hisab')} disabled={loading}>
                        Cancel
                      </button>
                    </div>
                  </div>
