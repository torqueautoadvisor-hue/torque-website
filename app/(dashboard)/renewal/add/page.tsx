...
import RenewalAddForm from './RenewalAddForm';

export default async function RenewalAddPage() {
  ...
  try {
    ...
    // 4. Pre-calculate sequential Renewal Code
    const countRes = await db.query("SELECT COUNT(*) as count FROM renewal_detail WHERE ren_code_no LIKE 'REN%'");
    const totalCount = parseInt(countRes.rows[0].count);
    nextCode = `REN${totalCount + 1}`;

  } catch (err: any) {
    console.error('Failed to load add renewal form metadata:', err.message);
  }
