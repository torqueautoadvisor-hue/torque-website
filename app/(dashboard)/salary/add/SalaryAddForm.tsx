...
export default function SalaryAddForm({
  staffList,
  branchId,
  isAdmin,
}: {
  ...
  // Form field states
  const [slrDate, setSlrDate] = useState(new Date().toISOString().split('T')[0]);
  const [admId, setAdmId] = useState('');
  const [slrFix, setSlrFix] = useState('0');
  const [slrPaid, setSlrPaid] = useState('0');
  const [slrDed, setSlrDed] = useState('0');
  const [slrPre, setSlrPre] = useState('0');
  const [slrAbs, setSlrAbs] = useState('0');
  const [slrStatus, setSlrStatus] = useState('1');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await createSalaryRecord(formData);
      if (res.success) {
        router.push(res.redirect || '/salary');
        router.refresh();
      } else {
  ...
  return (
    <div>
      <div className="pageheader">
        <h2><i className="fa fa-plus"></i> Add Salary</h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/insurance">Dashboard</a></li>
            <li><a href="/salary">Salary List</a></li>
            <li className="active">Add Salary</li>
          </ol>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="form-horizontal">
              <div className="panel panel-default">
                <div className="panel-heading">
                  <h4 className="panel-title">Salary Details</h4>
                  <p>Please enter employee salary information here.</p>
                ...
                <div className="panel-footer">
                  <div className="row">
                    <div className="col-sm-12 ml_15">
                      ...
                      <button type="button" className="btn btn-default" style={{ marginLeft: '10px' }} onClick={() => router.push('/salary')}>
                        Cancel
                      </button>
                    </div>
                  </div>
