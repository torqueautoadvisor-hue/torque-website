...
import { createTakenFollowupAction } from '../../../actions/taken';
...
export default function TakenFollowupForm({
  record,
  followups,
  flag = '',
}: {
  ...
  const [formData, setFormData] = useState({
    tkn_id: record.tkn_id.toString(),
    tknflp_date: new Date().toISOString().split('T')[0],
    tknflp_reminder_date: '',
    tknflp_notes: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.tknflp_date || !formData.tknflp_notes) {
      setError('Date and Notes are required.');
      return;
    }

    setLoading(true);
    const res = await createTakenFollowupAction(formData);
    setLoading(false);
  ...
  return (
    <div>
      <div className="pageheader">
        <h2><i className="fa fa-plus"></i> Follow Up - {record.tkn_name} ({record.tkn_code_no})</h2>
              ...
              <div className="panel panel-default">
                ...
                <div className="panel-body">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      type="date"
                      name="tknflp_reminder_date"
                      className="form-control"
                      value={formData.tknflp_reminder_date}
                      onChange={handleChange}
                    />
                  </div>
              ...
              <table className="table table-success mb30 table-hover table-bordered display" style={{ color: '#000' }}>
                ...
                <tbody>
                  {followups.length === 0 ? (
                    ...
                    followups.map((flp) => {
                      const reminderDateStr = flp.tknflp_reminder_date ? new Date(flp.tknflp_reminder_date).toISOString().split('T')[0] : '';
                      const isDueToday = reminderDateStr === todayStr;

                      return (
                        <tr key={flp.tknflp_id}>
                          <td>{formatDate(flp.tknflp_date)}</td>
                          <td
                            className={isDueToday ? 'blink_me' : ''}
                            style={isDueToday ? { color: 'red', fontWeight: 'bold' } : {}}
                          >
                            {flp.tknflp_reminder_date ? formatDate(flp.tknflp_reminder_date) : ''}
                          </td>
                          <td>{flp.tknflp_notes}</td>
