...
import TakenListClient from './TakenListClient';

export default async function TakenListPage({
  searchParams,
}: {
  ...
  try {
    ...
    // 2. Compose main Taken Logs query with dynamic date filters
    let query = `SELECT td.*, st.status_name, al.adm_username, rj.rej_res_name
                 FROM taken_detail td
                 JOIN status_detail st ON st.status_id = td.tkn_status
                 LEFT JOIN admin_login al ON al.adm_id = td.tkn_adm_id
                 LEFT JOIN rej_res_detail rj ON rj.rej_res_id = td.rej_res_id
                 WHERE td.tkn_status != 3 AND td.tkn_action IN ('1', '2') AND td.branch_id = $1`;

    const queryParams: any[] = [branchId];

    if (strdate && enddate) {
      let dateField = 'td.tkn_insurance_date';
      if (dateType === '2') dateField = 'td.tkn_cf_date';
      else if (dateType === '3') dateField = 'td.tkn_reg_date';
      else if (dateType === '4') dateField = 'td.tkn_permit_date';
      else if (dateType === '5') dateField = 'td.tkn_nat_permit_date';
      else if (dateType === '6') dateField = 'td.tkn_tax_date';
    ...
    if (!isAdmin) {
      ...
      query += ` AND td.tkn_adm_id = $${queryParams.length}`;
    }

    query += ` ORDER BY td.tkn_id`;

    const recordsRes = await db.query(query, queryParams);
    records = recordsRes.rows;

  } catch (err: any) {
    console.error('Failed to load taken records:', err.message);
  }
