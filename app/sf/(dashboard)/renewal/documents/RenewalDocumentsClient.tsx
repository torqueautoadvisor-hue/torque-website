...
import { uploadRenewalDocumentAction, deleteRenewalDocumentAction } from '../../../../actions/renewal';

export default function RenewalDocumentsClient({
  record,
  documents,
  flag = '',
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append('ren_id', record.ren_id.toString());
    formData.append('document_title', title);
    formData.append('document_image', file);

    const res = await uploadRenewalDocumentAction(formData);
    setUploading(false);
  ...
  const handleDelete = async (docId: number) => {
    ...
    const res = await deleteRenewalDocumentAction(docId, record.ren_id);
    setDeletingId(null);
  ...
  return (
    <div>
      <div className="pageheader">
        <h2>
          <i className="fa fa-plus"></i> Documents - {record.ren_name} ({record.ren_code_no})
        </h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/sf/insurance">Dashboard</a></li>
            <li><a style={{ color: '#1C1B17' }} href="/sf/renewal">Renewal List</a></li>
            <li className="active">Documents - {record.ren_name}</li>
          </ol>
        </div>
      </div>
              ...
              <div className="panel panel-default">
                <div className="panel-heading">
                  <h4 className="panel-title">
                    Upload Document - {record.ren_name} ({record.ren_code_no})
                  </h4>
                ...
                <div className="panel-footer">
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    <h3 style={{ margin: '0', fontWeight: 'bold' }}>
                      Uploaded Documents - {record.ren_name}
                    </h3>
                  </div>
