...
import GlobalListClient from './GlobalListClient';
...
export default async function GlobalListPage({
  ...
  try {
    ...
    // 2. Compose main Global Insurance query with dynamic date filters
    let query = `SELECT td.*, st.status_name, al.adm_username, rj.rej_res_name
                 FROM global_detail td
                 JOIN status_detail st ON st.status_id = td.glb_status
                 LEFT JOIN admin_login al ON al.adm_id = td.glb_adm_id
                 LEFT JOIN rej_res_detail rj ON rj.rej_res_id = td.rej_res_id
                 WHERE td.glb_status != 3 AND td.glb_action IN ('1', '2') AND td.branch_id = $1`;

    const queryParams: any[] = [branchId];

    if (strdate && enddate) {
      let dateField = 'td.glb_insurance_date';
      if (dateType === '2') dateField = 'td.glb_cf_date';
      else if (dateType === '3') dateField = 'td.glb_reg_date';
      else if (dateType === '4') dateField = 'td.glb_permit_date';
      else if (dateType === '5') dateField = 'td.glb_nat_permit_date';
      else if (dateType === '6') dateField = 'td.glb_tax_date';
    ...
    if (!isAdmin) {
      ...
      query += ` AND td.glb_adm_id = $${queryParams.length}`;
    }

    query += ` ORDER BY td.glb_id`;

    const recordsRes = await db.query(query, queryParams);
    records = recordsRes.rows;

  } catch (err: any) {
    console.error('Failed to load global insurance records:', err.message);
  }
