...
import { deleteTakenRecord, bulkActionTakenRecords } from '../../../actions/taken';

export default function TakenListClient({
  records,
  staffList,
  isAdmin,
  branchId,
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`/taken?strdate=${strdate}&enddate=${enddate}&date_type=${dateType}`);
  };
  ...
  const handleDownloadReport = (e: React.FormEvent) => {
    ...
    window.location.href = `/taken_export.php?strdate=${strdate}&enddate=${enddate}&date_type=${dateType}`;
  };

  // Checkbox management
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const allIds = filteredRecords.map((r) => r.tkn_id);
  ...
  const handleDeleteRecord = async (id: number) => {
    ...
    setExecuting(true);
    const res = await deleteTakenRecord(id);
    setExecuting(false);
    if (res.success) {
      router.refresh();
  ...
  const handleBulkActionSubmit = async (e: React.FormEvent) => {
    ...
    setExecuting(true);
    const res = await bulkActionTakenRecords(actionType, selectedIds);
    setExecuting(false);
  ...
  const getRecordDateValue = (row: any) => {
    let dateStr = row.tkn_date;
    if (dateType === '1') dateStr = row.tkn_insurance_date;
    else if (dateType === '2') dateStr = row.tkn_cf_date;
    else if (dateType === '3') dateStr = row.tkn_reg_date;
    else if (dateType === '4') dateStr = row.tkn_permit_date;
    else if (dateType === '5') dateStr = row.tkn_nat_permit_date;
    else if (dateType === '6') dateStr = row.tkn_tax_date;
  ...
  const filteredRecords = records.filter((row) => {
    ...
    return (
      (row.tkn_series || '').toLowerCase().includes(term) ||
      (row.tkn_reg_no || '').toLowerCase().includes(term) ||
      (row.tkn_name || '').toLowerCase().includes(term) ||
      (row.tkn_contact || '').toLowerCase().includes(term) ||
      (row.tkn_vmodel || '').toLowerCase().includes(term) ||
  ...
  return (
    <div>
      <div className="pageheader">
        <h2><i className="fa fa-table"></i> Taken List</h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/sf/insurance">Dashboard</a></li>
            <li className="active">Taken List</li>
          </ol>
        </div>
      </div>
      ...
      <div className="contentpanel">
        <div className="panel panel-default">
          <div className="panel-body">
            {/* Action Alert Banner */}
            {flag === '1' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Taken details added successfully.</p>}
            {flag === '2' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Taken details updated successfully.</p>}
            {flag === '3' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Taken details deleted successfully.</p>}
            {flag === '4' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Taken details assign action applied successfully.</p>}
            {flag === '6' && <p className="mb20" style={{ color: 'red', fontWeight: 'bold' }}>Taken details deleted successfully.</p>}
                  ...
                  <form method="POST" action="/taken_excelUpload.php" encType="multipart/form-data">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                      <h5 style={{ margin: '0', fontWeight: 'bold' }}>
                        Upload Taken Data by excel file <span className="asterisk">*</span>
                      </h5>
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        <input type="file" name="file" required className="form-control" style={{ height: '40px' }} />
                        <button type="submit" name="submit_taken_excel" className="btn btn-success" style={{ height: '40px' }}>
                          Upload
                        </button>
                      </div>
                      <a href="/images/Taken-Sample.xls" style={{ fontSize: '12px', color: 'green', textDecoration: 'underline' }}>
            ...
            <form onSubmit={handleBulkActionSubmit}>
              ...
              <div style={{ marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  ...
                  />
                  <a href="/sf/taken/add" className="btn btn-success" style={{ padding: '6px 12px' }}>
                    <i className="fa fa-plus"></i> Add Taken
                  </a>
                </div>
              </div>
                  ...
                  <tbody>
                    {paginatedRecords.length === 0 ? (
                      ...
                      paginatedRecords.map((row) => {
                        ...
                        return (
                          <tr key={row.tkn_id} className="odd gradeX">
                            {isAdmin && (
                              <td style={{ textAlign: 'center' }}>
                                <input
                                  type="checkbox"
                                  checked={selectedIds.includes(row.tkn_id)}
                                  onChange={(e) => handleSelectRow(row.tkn_id, e.target.checked)}
                                />
                              </td>
                            )}
                            <td>{row.tkn_series}</td>
                            <td>{getRecordDateValue(row)}</td>
                            <td>{row.tkn_reg_no}</td>
                            <td>
                              <a href={`/taken/edit?tkn_id=${row.tkn_id}`} style={{ color: 'green', textDecoration: 'underline' }}>
                                {row.tkn_name}
                              </a>
                            </td>
                            <td>{row.tkn_contact}</td>
                            <td>{row.tkn_vmodel}</td>
                            <td>{row.adm_username || ''}</td>
                            <td style={status.style}>{status.text}</td>
                            <td style={{ textAlign: 'center' }}>
                              <a href={`/taken/followup?tkn_id=${row.tkn_id}`} title="Follow Up" style={{ color: '#1C1B17', marginRight: '8px' }}>
                                <i className="fa fa-comment"></i>
                              </a>
                              <span style={{ color: '#ccc', marginRight: '8px' }}>|</span>
                              <a href={`/taken/documents?tkn_id=${row.tkn_id}`} title="Documents" style={{ color: '#333', marginRight: '8px' }}>
                                <i className="fa fa-image"></i>
                              </a>
                              <span style={{ color: '#ccc', marginRight: '8px' }}>|</span>
                              <a href={`/taken/edit?tkn_id=${row.tkn_id}`} title="Edit" style={{ color: 'green', marginRight: '8px' }}>
                              ...
                              {isAdmin && (
                                <>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => handleDeleteRecord(row.tkn_id)}
                                    title="Delete"
