...
import VahanListClient from './VahanListClient';

export default async function VahanListPage({
  searchParams,
}: {
  ...
  try {
    ...
    // 3. Compose main Vahan list query (service_id = 2)
    let query = `SELECT rd.*, st.status_name, al.adm_username
                 ...
                 WHERE rd.service_id = 2 AND rd.rto_status != 3 AND rd.branch_id = $1`;
    ...
    console.error('Failed to load vahan records:', err.message);
  }
