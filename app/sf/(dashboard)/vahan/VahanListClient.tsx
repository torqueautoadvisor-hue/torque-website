...
export default function VahanListClient({
  records,
  staffList,
  isAdmin,
  branchId,
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`/vahan?strdate=${strdate}&enddate=${enddate}`);
  };
  ...
  const handleDeleteRecord = async (id: number) => {
    ...
    setExecuting(true);
    const res = await deleteRtoRecord(id, 2); // serviceId = 2 (Vahan)
    setExecuting(false);
    if (res.success) {
      router.refresh();
  ...
  const handleBulkActionSubmit = async (e: React.FormEvent) => {
    ...
    setExecuting(true);
    const res = await bulkActionRtoRecords(2, actionType, selectedIds); // serviceId = 2 (Vahan)
    setExecuting(false);
  ...
  return (
    <div>
      <div className="pageheader">
        <h2><i className="fa fa-table"></i> Vahan List</h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/sf/rto">RTO Dashboard</a></li>
            <li className="active">Vahan List</li>
          </ol>
        </div>
      </div>
      ...
      <div className="contentpanel">
        ...
        <div className="panel panel-default">
          <div className="panel-body">
            {/* Action Alert Banner */}
            {flag === '1' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Vahan details added successfully.</p>}
            {flag === '2' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Vahan details updated successfully.</p>}
            {flag === '3' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Vahan details deleted successfully.</p>}
            {flag === '4' && <p className="mb20" style={{ color: 'green', fontWeight: 'bold' }}>Vahan details assign action applied successfully.</p>}
            {flag === '6' && <p className="mb20" style={{ color: 'red', fontWeight: 'bold' }}>Vahan details deleted successfully.</p>}
            ...
            <form onSubmit={handleBulkActionSubmit}>
              ...
              <div style={{ marginBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  ...
                  />
                  <a href="/sf/vahan/add" className="btn btn-success" style={{ padding: '6px 12px' }}>
                    <i className="fa fa-plus"></i> Add Vahan
                  </a>
                </div>
              </div>
                      ...
                      paginatedRecords.map((row) => {
                        ...
                        return (
                          <tr key={row.rto_id} className="odd gradeX">
                            ...
                            <td>
                              <a href={`/vahan/edit?rto_id=${row.rto_id}`} style={{ color: 'green', textDecoration: 'underline' }}>
                                {row.rto_name}
                              </a>
                            </td>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              <a href={`/vahan/documents?rto_id=${row.rto_id}`} title="Documents" style={{ color: '#333', marginRight: '8px' }}>
                                <i className="fa fa-image"></i>
                              </a>
                              <span style={{ color: '#ccc', marginRight: '8px' }}>|</span>
                              <a href={`/vahan/edit?rto_id=${row.rto_id}`} title="Edit" style={{ color: 'green', marginRight: '8px' }}>
