...
import { updateUghraniRecord } from '../../../../actions/ughrani';

export default function UghraniEditForm({
  record,
  branchId,
  isAdmin,
}: {
  ...
  // Form field states - Note the typo in database field name `ugh_descripiton`
  const [ughName, setUghName] = useState(record.ugh_name || '');
  const [ughContact, setUghContact] = useState(record.ugh_contact || '');
  const [ughDescription, setUghDescription] = useState(record.ugh_descripiton || '');
  const [ughAmount, setUghAmount] = useState(record.ugh_amount?.toString() || '');
  const [ughDate, setUghDate] = useState(formatDateForInput(record.ugh_date));
  const [ughDueDate, setUghDueDate] = useState(formatDateForInput(record.ugh_due_date));
  const [ughAction, setUghAction] = useState(record.ugh_action?.toString() || '1'); // 1 = Pending, 2 = Received
  const [ughStatus, setUghStatus] = useState(record.ugh_status?.toString() || '1'); // 1 = Active, 2 = Inactive
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateUghraniRecord(record.ugh_id, formData);
      if (res.success) {
        router.push(res.redirect || '/sf/ughrani');
        router.refresh();
      } else {
  ...
  return (
    <div>
      <div className="pageheader">
        <h2><i className="fa fa-pen"></i> Edit Ughrani Record</h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/sf/insurance">Dashboard</a></li>
            <li><a href="/sf/ughrani">Ughrani List</a></li>
            <li className="active">Edit Ughrani</li>
          </ol>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="form-horizontal">
              <div className="panel panel-default">
                <div className="panel-heading">
                  <h4 className="panel-title">Ughrani Collection Details ({record.ugh_code_no})</h4>
                  <p>Please edit client outstanding collection information here.</p>
                ...
                <div className="panel-footer">
                  <div className="row">
                    <div className="col-sm-12 ml_15">
                      ...
                      <button type="button" className="btn btn-default" style={{ marginLeft: '10px' }} onClick={() => router.push('/sf/ughrani')}>
                        Cancel
                      </button>
                    </div>
                  </div>
