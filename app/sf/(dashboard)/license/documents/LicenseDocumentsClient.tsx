...
import { uploadRtoDocumentAction, deleteRtoDocumentAction } from '../../../../actions/rto';

export default function LicenseDocumentsClient({
  rtoRecord,
  documents,
  rtoId,
  flag = '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append('rto_id', rtoId.toString());
    formData.append('service_id', '1'); // service_id = 1 (License)
    formData.append('document_title', documentTitle);
    formData.append('document_image', file);

    try {
      const res = await uploadRtoDocumentAction(formData);
      if (res.success) {
        setDocumentTitle('');
  ...
  const handleDeleteDoc = async (docId: number) => {
    ...
    const res = await deleteRtoDocumentAction(docId, rtoId, 1); // serviceId = 1
    if (res.success) {
      router.refresh();
      if (res.redirect) router.push(res.redirect);
    } else {
  ...
  return (
    <div>
      <div className="pageheader">
        <h2>
          <i className="fa fa-plus"></i> Documents - {rtoRecord.rto_code_no}({rtoRecord.rto_regno})
        </h2>
        <div className="breadcrumb-wrapper">
          <span className="label">You are here:</span>
          <ol className="breadcrumb">
            <li><a style={{ color: '#1C1B17' }} href="/sf/rto">RTO Dashboard</a></li>
            <li><a href="/sf/license">License List</a></li>
            ...
            <form onSubmit={handleSubmit} className="form-horizontal" encType="multipart/form-data">
              <div className="panel panel-default">
                <div className="panel-heading">
                  <h4 className="panel-title">Documents - {rtoRecord.rto_code_no}({rtoRecord.rto_regno})</h4>
                ...
                <div className="panel-footer">
                  <div className="row">
                    <div className="col-sm-9 col-sm-offset-3">
                      ...
                      <button type="button" className="btn btn-default" style={{ marginLeft: '10px' }} onClick={() => router.push('/sf/license')}>
                        Cancel
                      </button>
                    </div>
                  </div>
                  ...
                  <div style={{ marginLeft: '15px' }}>
                    <h3>Documents - {rtoRecord.rto_code_no}({rtoRecord.rto_regno})</h3>
                  </div>
                  <br />
