...
export default async function CategoryEditPage({
  ...
  if (!ctgIdStr) redirect('/sf/setup/categories');

  const ctgId = parseInt(ctgIdStr);
  if (isNaN(ctgId)) redirect('/sf/setup/categories');
  ...
  try {
    ...
    if (recRes.rows.length === 0) redirect('/sf/setup/categories');
