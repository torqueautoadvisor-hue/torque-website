...
export default async function CompanyEditPage({
  ...
  if (!cmpIdStr) redirect('/sf/setup/companies');

  const cmpId = parseInt(cmpIdStr);
  if (isNaN(cmpId)) redirect('/sf/setup/companies');
  ...
  try {
    ...
    if (recRes.rows.length === 0) redirect('/sf/setup/companies');
